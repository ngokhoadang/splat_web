...
async function main() {
    ...
    const url = new URL(
        params.get("url") || "3283_edit.splat",
        "https://huggingface.co/ngokhoadang2002/web_splat/resolve/main/",
    );
    ...
    window.addEventListener("keydown", (e) => {
        if (document.activeElement === document.getElementById("search")) return; // Vô hiệu hóa khi đang nhập liệu
        carousel = false;
    ...
    const frame = (now) => {
        ...
        let isJumping = false; // Initialize isJumping at the start of the frame

        if (document.activeElement !== document.getElementById("search")) { // Only process controls when not in input
            ...
            isJumping = activeKeys.includes("Space"); // Update isJumping based on Space key
            ...
            for (let gamepad of gamepads) {
                ...
                if (gamepad.buttons[0].pressed) {
                    isJumping = true; // Update isJumping for gamepad button
                    carousel = false;
                }
        ...
        if (document.activeElement !== document.getElementById("search")) { // Only process jumping when not in input
            if (isJumping) {
